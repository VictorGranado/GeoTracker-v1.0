/*
  GeoTracker V1.0
  Final Integration UI v2.2 Studio Sessions + Smooth TFT
  ...
  - Writes GeoTracker Studio compatible session folders automatically:
      /GEOTRK/S0001/session_info.csv
      /GEOTRK/S0001/samples.csv
      /GEOTRK/S0001/waypoints.csv
      /GEOTRK/S0001/events.csv
    A new folder is created at boot, and a new one is created on a later
    tracking start if the previous route already logged samples.
...
#define TFT_INTERVAL_MS    2000  // partial TFT refresh only; full redraw happens on screen changes
...
// GeoTracker creates Studio-compatible session folders automatically:
// /GEOTRK/S0001/session_info.csv
// /GEOTRK/S0001/samples.csv
// /GEOTRK/S0001/waypoints.csv
// /GEOTRK/S0001/events.csv
const char *baseFolder = "/GEOTRK";

String sessionId = "S0000";
String sessionFolder = "";
String sessionInfoFile = "";
String samplesFile = "";
String waypointFile = "";
String eventFile = "";
String statusFile = "";
String sessionStartUTC = "1970-01-01T00:00:00Z";
...
// TFT redraw state
// v2.1 avoids full-screen redraws every cycle.
// Full redraws happen only when the UI state changes.
// Periodic updates clear only the content region, keeping the header/footer stable.
bool tftNeedsFullRedraw = true;
bool tftPartialRefresh = false;
PageId tftDrawnPage = PAGE_COUNT;
PathView tftDrawnPathView = PATH_VIEW_MENU;
InputMode tftDrawnInputMode = INPUT_NONE;
...
String utcTimestampString() {
  ...
  // Studio expects a parseable UTC timestamp. This fallback keeps the
  // session importable if GPS time is not available yet.
  return "1970-01-01T00:00:00Z";
}

unsigned long sessionElapsedMs() {
...
void clearTFTPage() {
  ...
  if (tftPartialRefresh) {
    // Keep header and footer stable. Only the main content area is refreshed.
    tft.fillRect(0, 29, 320, 188, ILI9341_BLACK);
  } else {
...
String paddedSessionId(int number) {
  char buffer[8];
  snprintf(buffer, sizeof(buffer), "S%04d", number);
  return String(buffer);
}

void writeSessionInfoFile() {
  if (!sdOK || !sessionReady) return;

  if (sessionStartUTC == "1970-01-01T00:00:00Z" && gps.date.isValid() && gps.time.isValid()) {
    sessionStartUTC = utcTimestampString();
  }

  if (SD.exists(sessionInfoFile.c_str())) SD.remove(sessionInfoFile.c_str());

  File file = SD.open(sessionInfoFile.c_str(), FILE_WRITE);
  if (!file) return;

  file.println("key,value");
  file.println("schema_version,1.0");
  file.print("session_id,"); file.println(sessionId);
  file.println("device_name,GeoTracker v1.0");
  file.println("firmware_version,GeoTracker V1 Final UI v2.2 Studio Sessions + Smooth TFT");
  file.print("start_time_utc,"); file.println(sessionStartUTC);
  file.print("log_interval_ms,"); file.println(LOG_INTERVAL_MS);
  file.println("coordinate_system,WGS84");
  file.println("units,metric");

  file.close();
}

void createStudioCSVFiles() {
  if (!sdOK || !sessionReady) return;

  if (!SD.exists(sessionInfoFile.c_str())) {
    writeSessionInfoFile();
  }

  if (!SD.exists(samplesFile.c_str())) {
    File file = SD.open(samplesFile.c_str(), FILE_WRITE);
    if (file) {
      file.println("sample_id,timestamp_utc,elapsed_ms,gps_valid,gps_updated,gps_fix_type,satellites,latitude_deg,longitude_deg,gps_altitude_m,speed_mps,course_deg,hdop,pdop,vdop,bme_valid,temperature_c,humidity_pct,pressure_hpa,pressure_altitude_m,mag_valid,mag_x_ut,mag_y_ut,mag_z_ut,mag_strength_ut,heading_deg,imu_valid,accel_x_mps2,accel_y_mps2,accel_z_mps2,gyro_x_dps,gyro_y_dps,gyro_z_dps,pitch_deg,roll_deg");
      file.close();
    }
  }

  if (!SD.exists(waypointFile.c_str())) {
    File file = SD.open(waypointFile.c_str(), FILE_WRITE);
    if (file) {
      file.println("waypoint_id,source_sample_id,timestamp_utc,elapsed_ms,latitude_deg,longitude_deg,gps_altitude_m,heading_deg,label,note");
      file.close();
    }
  }

  if (!SD.exists(eventFile.c_str())) {
    File file = SD.open(eventFile.c_str(), FILE_WRITE);
    if (file) {
      file.println("event_id,source_sample_id,timestamp_utc,elapsed_ms,event_type,latitude_deg,longitude_deg,gps_altitude_m,value,note");
      file.close();
    }
  }
}
...
void createNewSessionFolder() {
  ...
  if (!SD.exists(baseFolder)) SD.mkdir(baseFolder);

  sessionReady = false;

  for (int i = 1; i <= 9999; i++) {
    String candidateId = paddedSessionId(i);
    String candidateFolder = String(baseFolder) + "/" + candidateId;

    if (!SD.exists(candidateFolder.c_str())) {
      if (!SD.mkdir(candidateFolder.c_str())) {
        Serial.println("Failed to create session folder.");
        return;
      }

      sessionId = candidateId;
      sessionFolder = candidateFolder;
      sessionInfoFile = sessionFolder + "/session_info.csv";
      samplesFile = sessionFolder + "/samples.csv";
      waypointFile = sessionFolder + "/waypoints.csv";
      eventFile = sessionFolder + "/events.csv";
      statusFile = sessionFolder + "/STATUS.TXT";
      sessionStartUTC = utcTimestampString();
      sessionReady = true;

      // A new folder means a clean Studio session.
      logRow = 0;
      waypointCount = 0;
      eventCount = 0;

      createStudioCSVFiles();

      Serial.print("Created Studio session folder: ");
      Serial.println(sessionFolder);
      return;
    }
  }

  Serial.println("No free session folder slot found.");
}

void createStatusFile() {
  if (!sdOK || !sessionReady) return;

  File file = SD.open(statusFile.c_str(), FILE_WRITE);
  if (!file) return;

  file.println("GeoTracker V1.0 - Final UI v2.2 Studio Sessions + Smooth TFT");
  file.println("Subsystem status at boot:");
  file.print("Session folder: "); file.println(sessionFolder);
  ...
  file.println("Studio schema: session_info.csv, samples.csv, waypoints.csv, events.csv");
  file.close();
}
...
void readGPS() {
  ...
  if (gsaHDOP.isValid()) gpsHDOP = atof(gsaHDOP.value());
  if (gsaPDOP.isValid()) gpsPDOP = atof(gsaPDOP.value());
  if (gsaVDOP.isValid()) gpsVDOP = atof(gsaVDOP.value());
}
...
void saveEvent(const char *eventName, const String &note) {
  ...
  File file = SD.open(eventFile.c_str(), FILE_APPEND);
  if (!file) {
  ...
  file.print(logRow);
  file.print(",");
  file.print(utcTimestampString());
  file.print(",");
  file.print(sessionElapsedMs());
  file.print(",");
  file.print(eventName);
  file.print(",");

  if (gpsFix) file.print(gpsLat, 6);
  file.print(",");
  if (gpsFix) file.print(gpsLon, 6);
  file.print(",");
  if (gpsFix) file.print(gpsAltM, 1);
  file.print(",");
  file.print(eventName);
  file.print(",");
...
void saveWaypoint(const String &name, const String &source, double wptLat, double wptLon, double wptAlt, const String &note) {
  ...
  File file = SD.open(waypointFile.c_str(), FILE_APPEND);
  if (!file) {
    setStatusMessage("Waypoint file fail", 3000);
    Serial.println("Failed to open waypoints.csv");
    return;
  }
  ...
  file.print(",");
  file.println(csvSafe(note + String(" | source=") + source));
...
bool loadWaypointById(long wantedId, double &outLat, double &outLon, double &outAlt, String &outName) {
  if (!sdOK || !sessionReady) return false;

  deselectSPI();

  File file = SD.open(waypointFile.c_str());
  ...
  while (file.available()) {
    ...
    if (id == wantedId) {
      outLat = getCsvField(line, 4).toDouble();
      outLon = getCsvField(line, 5).toDouble();
      outAlt = getCsvField(line, 6).toDouble();
      outName = getCsvField(line, 8);
      file.close();
      return true;
    }
  }
...
String fileSizeString(const String &path) {
  if (!sdOK || !sessionReady) return "--";
...
void startTracking() {
  ...
  // If the previous route in this boot already produced samples, start a
  // fresh Studio session folder for the new route. This prevents multiple
  // recorded tracks from mixing inside one samples.csv file.
  if (sdOK && sessionReady && logRow > 0) {
    createNewSessionFolder();
    createStatusFile();
  }
  ...
  if (sessionReady && gps.date.isValid() && gps.time.isValid()) {
    sessionStartUTC = utcTimestampString();
    writeSessionInfoFile();
  }
  resetTrackStats();
...
void logToSD() {
  ...
  File file = SD.open(samplesFile.c_str(), FILE_APPEND);
  if (!file) {
    Serial.println("Failed to open samples.csv for append.");
    setStatusMessage("samples.csv append fail", 3000);
    return;
  }

  bool validGpsSample = gpsFix;

  file.print(logRow); file.print(",");
  file.print(utcTimestampString()); file.print(",");
  file.print(sessionElapsedMs()); file.print(",");
  file.print(validGpsSample ? 1 : 0); file.print(",");
  file.print(validGpsSample ? 1 : 0); file.print(",");
  file.print(gpsFixString()); file.print(",");
  file.print(gpsSatellites); file.print(",");

  if (validGpsSample) file.print(gpsLat, 6);
  file.print(",");
  if (validGpsSample) file.print(gpsLon, 6);
  file.print(",");
  if (validGpsSample) file.print(gpsAltM, 1);
  file.print(",");
  if (validGpsSample) file.print(gpsSpeedKmph / 3.6, 2);
  file.print(",");
  if (validGpsSample) file.print(gpsCourseDeg, 2);
  file.print(",");
  if (validGpsSample) file.print(gpsHDOP, 2);
  file.print(",");
  if (validGpsSample) file.print(gpsPDOP, 2);
  file.print(",");
  if (validGpsSample) file.print(gpsVDOP, 2);
  file.print(",");

  file.print(bmeOK ? 1 : 0); file.print(",");
  ...
  file.print(bmmOK ? 1 : 0); file.print(",");
  ...
  file.print(mpuOK ? 1 : 0); file.print(",");
  file.print(accelX, 2); file.print(",");
  file.print(accelY, 2); file.print(",");
  file.print(accelZ, 2); file.print(",");
  file.print(gyroX, 2); file.print(",");
  file.print(gyroY, 2); file.print(",");
  file.print(gyroZ, 2); file.print(",");
  file.print(pitchDeg, 2); file.print(",");
  file.println(rollDeg, 2);
...
void drawPathInputScreen() {
  ...
  if (inputMode == INPUT_TARGET_LAT || inputMode == INPUT_TARGET_LON) {
    ...
    tft.print("Load saved waypoint from waypoints.csv by ID.");
  }
...
void drawPathMenu() {
  ...
  if (pathMenuIndex == PATH_LIVE_GPS) {
    ...
    tft.setCursor(126, 86); tft.println("from waypoints.csv.");
...
void drawLoggingScreen() {
  String logSize = fileSizeString(samplesFile);
  ...
  tft.setTextColor(ILI9341_WHITE);
  tft.setCursor(10, 58); tft.print("samples.csv     "); tft.print(logSize);
  tft.setCursor(10, 74); tft.print("waypoints.csv  "); tft.print(wptSize);
...
bool tftViewStateChanged() {
  return currentPage != tftDrawnPage ||
         pathView != tftDrawnPathView ||
         inputMode != tftDrawnInputMode;
}

void rememberTFTViewState() {
  tftDrawnPage = currentPage;
  tftDrawnPathView = pathView;
  tftDrawnInputMode = inputMode;
}

bool shouldAutoRefreshTFT() {
  // The OLEDs already handle fast live compass/GPS feedback.
  // TFT updates are intentionally calmer.
  if (inputMode != INPUT_NONE) return false;

  if (currentPage == PAGE_HOME) return true;
  if (currentPage == PAGE_ENVIRONMENT) return true;
  if (currentPage == PAGE_LOGGING) return true;
  if (currentPage == PAGE_MOVEMENT) return true;

  if (currentPage == PAGE_PATH_TRACKING) {
    return pathView == PATH_VIEW_LIVE_GPS ||
           pathView == PATH_VIEW_TRACKING_CONTROL ||
           pathView == PATH_VIEW_NAV_ACTIVE ||
           pathView == PATH_VIEW_RETURN_START;
  }
...
void serviceTFT() {
  ...
  bool fullRedraw = tftNeedsFullRedraw || tftViewStateChanged();

  if (fullRedraw) {
    tftPartialRefresh = false;
    drawCurrentTFTScreen();
    tftPartialRefresh = false;
    tftNeedsFullRedraw = false;
    rememberTFTViewState();
    lastTFTUpdate = millis();
    return;
  }

  if (shouldAutoRefreshTFT() && millis() - lastTFTUpdate >= TFT_INTERVAL_MS) {
    tftPartialRefresh = true;
    drawCurrentTFTScreen();
    tftPartialRefresh = false;
    rememberTFTViewState();
    lastTFTUpdate = millis();
...
void setup() {
  ...
  Serial.println();
  Serial.println("========================================");
  Serial.println(" GeoTracker V1.0 Final UI v2.2 Studio Sessions + Smooth TFT");
  Serial.println("========================================");
  ...
  saveEvent("BOOT", "GeoTracker UI v2.2 Studio sessions + smooth TFT started");

  Serial.println("GeoTracker V1.0 Final UI v2.2 Studio sessions + smooth TFT running.");
}
